...
`citizen1`-`citizen5` (*Pixel Art Market Square*) — two of which, `citizen1`
and `citizen5`, are now the player themself, one per gender — and the player's
old base body (*Free Base 4-Direction Male / Female Character*), still shipped
as the Custom option, whose clothing, hair and colours are generated by the
game rather than drawn. And the
