...
| `armorandweaponsicons` | **Assessed and rejected.** The earlier note here — "uniform icon grids" — was written from the file listing. Looked at: `Icons.png` is 200-odd helmets, breastplates, swords, axes, maces, spears and shields; `Furniture.png` is armour racks and weapon tables, which are shop props rather than item icons. This game has no armour and no weapons as items, and its 30 remaining icon gaps are cooked dishes, jungle produce, pelts and junk. Nothing here maps. |
